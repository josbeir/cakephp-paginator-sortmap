 ...
 */
namespace Josbeir\Paginator\Datasource;
...
class Paginator implements PaginatorInterface
{
     ...
     * sorting on either associated columns or calculated fields. In these cases you
     * can use the `sortMap` option to define a list of fields that need to be sorted on yourself
     *
     * ```
     * $settings = [
     *   'Articles' => [
     *     'finder' => 'custom',
     *     'sortMap' => ['title', 'author_id', 'comment_count'],
     *   ]
     * ];
     * ```
     *
     * Passing an empty array as sortMap disallows sorting altogether.
     *
    ...
    public function validateSort(RepositoryInterface $object, array $options)
    {
        ...
        if (isset($options['sortMap'])) {
            $options['order'] = $this->_buildSortMapping($object, $options);
        } else {
            $options['order'] = $this->_prefix($object, $options['order']);
        }
    ...
    /**
     * Generate sort conditions from the sortMap config
     *
     * @param \Cake\Datasource\RepositoryInterface $object Repository object.
     * @param array $options The pagination options being used for this request.
     *
     * @return array An array containing sort conditions based on provided mapping
     */
    protected function _buildSortMapping($object, array $options)
    {
        $map = $options['sortMap'] ?? [];
        $order = $options['order'];
        $sort = $options['sort'];
    ...
    /**
     * Prefixes a single field with the table alias if possible.
     *
     * @param \Cake\Datasource\RepositoryInterface $object Repository object.
     * @param string $field Name of the field
     * @param bool $mapped Whether or not the field was mapped.
     * @return string|null Prefixed field or null when no prefix could be returned
     */
    protected function _prefixField($object, string $field, $mapped = false)
    {
    ...
    /**
     * Prefixes all fields with the table alias if possible.
     *
     * @param \Cake\Datasource\RepositoryInterface $object Repository object.
     ...
     */
    protected function _prefix(RepositoryInterface $object, $order, $mapped = false)
    {
        $tableOrder = [];
        foreach ($order as $key => $value) {
            ...
            $field = $this->_prefixField($object, $key, $mapped);
            if ($field) {
